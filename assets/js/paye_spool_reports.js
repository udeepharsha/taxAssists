	...
	query.first({
		success: function(resultsObj) {
			if(resultsObj){
                ...
                calculateCumulativeFiguresByEmployee(employee, returnMonthValue(resultsObj.get('month')), resultsObj.get('year'));
			}
		},
		error: function(error) {
			// alert("Please check your internet connection!");
		...
		success: function(results) {
			for (var i = 0; i < results.length; i++) {
				...
				if(getMonthVal <= currentMonthValue){
					var monthlyValues1 = obj.get('monthlyValues');
...
function downloadAnnualExcelReport2(identity) {
	document.getElementById("generatingExcelYear").innerHTML = "<img class='loadingImage' src='images/loading1.gif' />";
...
function exportDataYearly(){
	...
	$.ajax({
        ...
        success: function(result) {
            ...
            document.getElementById("generatingExcelYear").innerHTML = "<img class='doneImage' src='images/done.png' />";
        }
    });
}
